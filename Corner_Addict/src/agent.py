...
class Agent:
	...
	# 	with torch.no_grad():
	# 		action_idx, probs = self.policy.get_action(state, deterministic=deterministic)
	# 	if deterministic:
	# 		return self.action_parser.parse_actionss([action_idx], None)[0]
	# 	else:
	# 		return self.action_parser.parse_actionss([action_idx], None)[0][0]
	def act(self, state):
		with torch.no_grad():
